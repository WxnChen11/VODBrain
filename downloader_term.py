...
try:
    ...
    if app.pargs.url:
        ...
        if not os.path.isdir(app.pargs.output):
            os.makedirs(app.pargs.output)
        ...
        subprocess.call('wget -i %s -nv -O %s' % ('chunks.txt', transport_stream_file_name), cwd=app.pargs.output, shell=True)
